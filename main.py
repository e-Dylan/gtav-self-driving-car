...
file_name = "training_data_big.npy"
...
def main():
    ...
    while (True): # hook
        ...
        keys = keycheck() # get all keys being pressed
        output = keys_to_output(keys) # convert all active pressed keys to 1-hot vectors [1, 0, 0] => [A, 0, 0] => turning left
        training_data.append([screen, output]) # save the screen image, and the recorded "correct" outputs in training data.

        if len(training_data) % 500 == 0:
            # training_data has reached a multiple of 500 (been long enough that we want to save)
